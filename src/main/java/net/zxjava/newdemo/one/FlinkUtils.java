...
public class FlinkUtils {
	...
	public static <T> DataStream<T> createKafkaStream(ParameterTool parameters,
		...
		// FsStateBackend("file:\\\\lei_test_project\\idea_workspace\\FlinkTutorial\\check_point_dir"));
		// 取消任务checkPoint不删除
		...
		// KafkaSource
		FlinkKafkaConsumer011<T> kafkaConsumer = new FlinkKafkaConsumer011<T>(topicList, clazz.newInstance(), props);
	...
	public static <T> DataStream<T> createKafkaStream(ParameterTool parameters, String topics, String groupId,
			Class<? extends DeserializationSchema<T>> clazz) throws IllegalAccessException, InstantiationException {
		// 设置全局的参数，以后在自种算子里就可以拿到全局参数
		env.getConfig().setGlobalJobParameters(parameters);
		...
		// 设置StateBackend，生产环境不建议写在这里，建议写在flink-conf.yaml配置文件中；默认保存到内存里面
		// env.setStateBackend(new
		// FsStateBackend("ile:\\\\lei_test_project\\idea_workspace\\FlinkTutorial\\check_point_dir"));
		// 取消任务checkPoint不删除
		env.getCheckpointConfig()
				.enableExternalizedCheckpoints(CheckpointConfig.ExternalizedCheckpointCleanup.RETAIN_ON_CANCELLATION);
		// 设置checkPoint的模式, 上述指定此处不需要再次指定
