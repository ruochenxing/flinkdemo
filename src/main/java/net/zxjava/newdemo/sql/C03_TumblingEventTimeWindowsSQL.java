...
public class C03_TumblingEventTimeWindowsSQL {
	public static void main(String[] args) throws Exception {
		...
		SingleOutputStreamOperator<Row> waterMarkRow = rowDataStream
				.assignTimestampsAndWatermarks(new BoundedOutOfOrdernessTimestampExtractor<Row>(Time.seconds(0)) {
					private static final long serialVersionUID = -3730386384158534090L;
		...
		tableEnv.registerDataStream("t_orders", waterMarkRow, "etime, uid, pid, money, rowtime.rowtime");

		// HOP 是固定的，代表滑动窗口
		Table table = tableEnv
				.sqlQuery("SELECT uid, SUM(money), HOP_END(rowtime, INTERVAL '2' SECOND, INTERVAL '10' SECOND)"
