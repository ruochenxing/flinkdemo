...
public class C03_TumblingEventTimeWindowsTable {
	public static void main(String[] args) throws Exception {
		...
		SingleOutputStreamOperator<Row> rowDataStream = socketTextStream.map(new MapFunction<String, Row>() {
			private static final long serialVersionUID = -8431067267098002628L;
		...
		SingleOutputStreamOperator<Row> waterMarkRow = rowDataStream
				.assignTimestampsAndWatermarks(new BoundedOutOfOrdernessTimestampExtractor<Row>(Time.seconds(0)) {
					private static final long serialVersionUID = -8550764568498393835L;
		...
		// win.end 10000
		// win.rowtime 9999
		Table table = tableEnv.scan("t_orders").window(Tumble.over("10.seconds").on("rowtime").as("win")) // define
																											// window
